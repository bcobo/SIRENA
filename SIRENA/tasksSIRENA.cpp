...
* - If PRODUCTION ('mode=1') => Find pulses by 'InitialTriggering' and 'FindSecondaries' or 'FindSecondariesSTC'
*
* - Declare variables
...
int createLibrary(ReconstructInitSIRENA* reconstruct_init, bool *appendToLibrary, fitsfile **inLibObject, int inputPulseLength)
{
    ...
    else	// If the library does not exist yet => Create it
    {
        *appendToLibrary = false;
        ...
        char str_mode[125];
        snprintf(str_mode,125,"%d",reconstruct_init->mode);
        ...
        char str_LagsOrNot[125];
        snprintf(str_LagsOrNot,125,"%d",reconstruct_init->LagsOrNot);
        ...
        char str_OFIter[125];
        snprintf(str_OFIter,125,"%d",reconstruct_init->OFIter);
        ...
        char str_OFLib[125];
        snprintf(str_OFLib,125,"%d",reconstruct_init->OFLib);
        ...
        char str_OFLength[125];
        snprintf(str_OFLength,125,"%d",reconstruct_init->OFLength);
        strproc=string("OFLength = ") + string(str_OFLength);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_maxPulsesPerRecord[125];
        snprintf(str_maxPulsesPerRecord,125,"%d",reconstruct_init->maxPulsesPerRecord);
        strproc=string("maxPulsesPerRecord = ") + string(str_maxPulsesPerRecord);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_pulse_length[125];
        snprintf(str_pulse_length,125,"%d",inputPulseLength);
        strproc=string("PulseLength = ") + string(str_pulse_length);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_scaleFactor[125];
        snprintf(str_scaleFactor,125,"%f",reconstruct_init->scaleFactor);
        strproc=string("scaleFactor = ") + string(str_scaleFactor);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_samplesUp[125];
        snprintf(str_samplesUp,125,"%f",reconstruct_init->samplesUp);
        ...
        char str_nSgms[125];
        snprintf(str_nSgms,125,"%f",reconstruct_init->nSgms);
        ...
        char str_LrsT[125];
        snprintf(str_LrsT,125,"%e",reconstruct_init->LrsT);
        ...
        char str_LbT[125];
        snprintf(str_LbT,125,"%e",reconstruct_init->LbT);
        strproc=string("LbT = ") + string(str_LbT);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_monoenergy[125];
        snprintf(str_monoenergy,125,"%f",reconstruct_init->monoenergy);
        strproc=string("monoenergy = ") + string(str_monoenergy);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_hduPRECALWN[125];
        snprintf(str_hduPRECALWN,125,"%d",reconstruct_init->hduPRECALWN);
        strproc=string("hduPRECALWN = ") + string(str_hduPRECALWN);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_hduPRCLOFWM[125];
        snprintf(str_hduPRCLOFWM,125,"%d",reconstruct_init->hduPRCLOFWM);
        strproc=string("hduPRCLOFWM = ") + string(str_hduPRCLOFWM);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_largeFilter[125];
        snprintf(str_largeFilter,125,"%d",reconstruct_init->largeFilter);
        strproc=string("largeFilter = ") + string(str_largeFilter);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_intermediate[125];
        snprintf(str_intermediate,125,"%d",reconstruct_init->intermediate);
        ...
        char str_clobber[125];
        snprintf(str_clobber,125,"%d",reconstruct_init->clobber);
        strproc=string("clobber = ") + string(str_clobber);
        strncpy(keyvalstr,strproc.c_str(),999);
        keyvalstr[999]='\0';
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);

        char str_SaturationValue[125];
        snprintf(str_SaturationValue,125,"%e",reconstruct_init->SaturationValue);
...
int loadRecord(TesRecord* record, double *time_record, gsl_vector **adc_double)
{
    ...
    *time_record = record->time;
        ...
        // Baseline could be near to zero, even having negative values of the record
    {
        ...
        {
            EP_PRINT_ERROR("Last record size is 0",EPFAIL);
            return(EPFAIL);
        }
...
int procRecord(ReconstructInitSIRENA** reconstruct_init, double tstartRecord, double samprate, fitsfile *dtcObject, gsl_vector *record, PulsesCollection *foundPulses)
{
    ...
    for (int i=0; i<numPulses; i++)
    {
        ...
        if (gsl_vector_get(tendgsl,i) >= recordDERIVATIVE->size)		// Truncated pulses at the end of the record
        {
            ...
            gsl_vector_set(tendgsl,i,recordDERIVATIVE->size);
    ...
    if (numPulsesRecord != 0)
    {
        ...
        for (int i=0; i<numPulsesRecord; i++)
        {
            ...
            else 									// The invectorNOTFIL has less bins than sizePulse (truncated)
            {
                ...
                for (int j=(invectorNOTFIL->size)-t0; j< (*reconstruct_init)->pulse_length; j++) {
                    gsl_matrix_set (vgslout2,i,j,0.0);
                }
            }
        }
    ...
    else
    {
        ...
        if ((invectormax != 0) && (invectormin != 1e10))
        {
            ...
            for (int i=0; i<nbins; i++)	{
                gsl_vector_set (*xhistogsl,i,binSize*i+invectormin+binSize/2);
            }
...
int writeLibrary(ReconstructInitSIRENA **reconstruct_init, double samprate, double estenergy, gsl_vector *pulsetemplate, gsl_matrix *covariance, gsl_matrix *weight, bool appendToLibrary, fitsfile **inLibObject, gsl_vector *pulsetemplateMaxLengthFixedFilter)
{
    ...
    if (appendToLibrary == true)
    {
        ...
        snprintf(str_procnumber,125,"%ld",eventcntLib);
        ...
        char str_pulse_length[125];
        snprintf(str_pulse_length,125,"%d",(*reconstruct_init)->pulse_length);
        strproc=string("PulseLength = ") + string(str_pulse_length);
        strcpy(keyvalstr,strproc.c_str());
        fits_write_key(*inLibObject,TSTRING,keyname,keyvalstr,NULL,&status);
...
void runEnergy(TesRecord* record,ReconstructInitSIRENA** reconstruct_init, PulsesCollection** pulsesInRecord, OptimalFilterSIRENA **optimalFilter)
{
    ...
    /*cout<<"record"<<endl;
    cout<<"995 "<<record->adc_double[995]<<endl;
    cout<<"996 "<<record->adc_double[996]<<endl;
    cout<<"997 "<<record->adc_double[997]<<endl;
    cout<<"998 "<<record->adc_double[998]<<endl;
    cout<<"999 "<<record->adc_double[999]<<endl;
    cout<<"1000 "<<record->adc_double[1000]<<endl;
    cout<<"1001 "<<record->adc_double[1001]<<endl;
    cout<<"1002 "<<record->adc_double[1002]<<endl;
    cout<<"1003 "<<record->adc_double[1003]<<endl;
    cout<<"1004 "<<record->adc_double[1004]<<endl;
    cout<<"1005 "<<record->adc_double[1005]<<endl;*/
            ...
            // OPTFILT	I2R or I2RALL or I2RNOL or I2RFITTED => OPTFILT
        {
            ...
            else
            {
                ...
                for (int i=0; i<numlags; i++)
                {
                    ...
                    //cout<<gsl_vector_get(lags_vector,i)<<endl;
                }
            }
                    ...
                    else
                    {
                        ...
                        if ((xmax < -1) || (xmax > 1))
                        {
                            do
                            {
                                ...
                                {
                                    newEnergy = newEnergy + gsl_vector_get(vector,numlagsOUT/2+newLag+k)*gsl_vector_get(filter,k);
                    ...
                    else
                    {
                        ...
                        if ((xmax < -1) || (xmax > 1))
                        {
                            do
                            {
                                ...
                                gsl_vector  *vectorSHORT = gsl_vector_alloc(filterFFT->size);
                                temp = gsl_vector_subvector(vector,numlagsOUT/2+newLag,filterFFT->size);
                                gsl_vector_memcpy(vectorSHORT,&temp.vector);
                                if (FFT(vectorSHORT,vectorFFT,SelectedTimeDuration))
                                {
                                    message = "Cannot run routine FFT";
...
int interpolateFilter(double shift, int lengthInterp, gsl_vector *filter, gsl_vector_complex *filterFFT,gsl_vector **filterInterp, gsl_vector_complex **filterFFTInterp, int domain)
{
    ...
    else if (domain == 1)   // Frequency domain
    {
...
/*xxxx end of SECTION Bxx xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*/
