####################################################################################
# Runs the reconstruction of file of pairs of events with 'tesrecons' tool.        #
# optimal filter to check with Noise Matrix                                      #
...
def test_recons_pairs_optfilt_wn():
    """
    Test the 'tesrecons' tool with the OPTFILT energy method and WEIGHT matrix noise.
    This function tests the 'tesrecons' tool with the OPTFILT energy method + WEIGHTN
    ...
    file_output = f"{outdir}/events_pairs_3keV_6keV_sep3500samples_optfilt_wn.fits"
    library = f"{reference}/library_6keV_optfilt_wn_reference_{date_config_file}.fits"
    ...
    file_reference = f"{reference}/events_pairs_3keV_6keV_sep3500samples_optfilt_wn_reference_{date_config_file}.fits"
    ...
    assert output_fdiff.returncode == 0, f"fdiff failed to run with command: {comm}"
