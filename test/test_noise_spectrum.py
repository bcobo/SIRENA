################################################################################
# Runs the generation of a noise spectrum with 'gennoisespec'
...
def test_gennoisespec_psd():
    """
    Test the gennoisespec function.

    This function tests the gennoisespec function by performing the following steps:
    1. Define input and output files.
    2. Run the gennoisespec tool to create a noise spectrum 
    ...
    ns_input = f"{indir}/noise_{date_config_file}.fits"
    ns_output = f"{outdir}/noise_spectrum.fits"
    ...
    ref_file = f"{reference}/noise_spectrum_reference_{date_config_file}.fits"
